...
def mAP(c_x0,x_x1,c_y0,x_y1,model,k,s):
	metric = K.function(inputs=[model.layers[-5].output,model.layers[-4].output],outputs=[model.layers[-1].output])
...
def get_desc(text,image,model):
	...
	descriptor = Model(inputs=model.input,outputs=[model.layers[-5].output,model.layers[-4].output])
