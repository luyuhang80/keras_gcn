...
def get_desc(text,image,model):
	...
	descriptor1 = Model(inputs=[model.layers[0].input],outputs=[model.layers[-4].output])
	descriptor2 = Model(inputs=[model.layers[1].input],outputs=[model.layers[-3].output])
