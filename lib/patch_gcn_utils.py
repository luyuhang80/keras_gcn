...
def load_features(data_path,filelist):
    data = []
    for i in filelist:
        data.append(np.load(data_path+i))
    return np.array(data)
...
def prepair_data(train_val,data_path):
    ...
    textlist = get_list(data_path+'text_list.txt')
    text = load_features(data_path+'text_bow_10055/',textlist)
    text_label = np.load(data_path + '/load_ny0.npy').astype(int) - 1

    imagelist = get_list(data_path+'image_list.txt')
    image = load_features(data_path+'image_vgg19_pool5/',imagelist)
